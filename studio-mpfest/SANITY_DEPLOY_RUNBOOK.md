# MPFest Sanity Deploy Runbook

This runbook covers deploying the Sanity Studio, seeding current app content, and understanding how content updates reach the tablet app.
...
Force image re-upload only when necessary:

```powershell
npm run seed:current -- --replace-assets
```

The seed script uses stable IDs like `seed-era-golden_age`, `seed-knowledge-c1`, and `seed-event-golden_age-1635-0-0`, so rerunning it updates seeded documents instead of duplicating them.
...
- Intern edits appear after publishing and refreshing/restarting the app.
...
5. Confirm the app shows one of these messages:
   ...
   - `Wifi unavailable`
6. Confirm the edited card, detail page, or hotspot appears.
...
- Confirm `Era`, `Knowledge Item`, and `Timeline Event` appear in Structure.
...
- Is the edited item linked to the right Era or Timeline Event?
...
The script seeds documents with deterministic IDs and links related content after all Knowledge Items exist.
